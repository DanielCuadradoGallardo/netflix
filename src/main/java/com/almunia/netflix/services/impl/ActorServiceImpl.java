...
public class ActorServiceImpl implements ActorService {
    ...
    @Override
    public ActorDto getActorById(int id) throws NetflixException {
        return modelMapper.map(actorRepository.findById(id).orElseThrow(() -> new NotFoundException(ExceptionConstants.ACTOR_NOT_FOUND)), ActorDto.class);
    }

    @Override
    public ActorDto getActorByName(String name) throws NetflixException {
        return modelMapper.map(actorRepository.findActorByName(name).orElseThrow(() -> new NotFoundException(ExceptionConstants.ACTOR_NOT_FOUND)), ActorDto.class);
    }

    @Override
    public ActorDto createActor(ActorDto actorDto) throws NetflixException {
        ...
        if (actorRepository.findActorByName(actor.getName()).isPresent()) {
            throw new AlreadyExistsException(ExceptionConstants.ACTOR_ALREADY_EXISTS);
        } else {
    ...
    @Override
    public ActorDto deleteActor(int id) throws NetflixException {
